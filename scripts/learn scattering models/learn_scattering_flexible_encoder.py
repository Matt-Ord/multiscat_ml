...
class ForwardModel(nn.Module):
    ...
    def __init__(  # ruff: ignore[too-many-positional-arguments]   # ruff: ignore[too-many-arguments]
        ...
        self.global_encoder = GlobalPotentialEncoder(encoder_dim)
...
def _make_channel_index(nx: int, ny: int, device: torch.device) -> torch.Tensor:
    """Integer (n, m) storage indices in FFT order, shape (Nx * Ny, 2)."""
    ns = torch.fft.fftfreq(nx, d=1.0 / nx).round().long().to(device)
    ms = torch.fft.fftfreq(ny, d=1.0 / ny).round().long().to(device)
    grid = torch.stack(torch.meshgrid(ns, ms, indexing="ij"), dim=-1)
    return grid.reshape(nx * ny, 2)


def _trapezoid_weights(z: torch.Tensor) -> torch.Tensor:
    """Trapezoid quadrature weights for an arbitrary 1-D z grid."""
...
def _make_coords(
    ...
    zs = torch.linspace(0.0, Z_HEIGHT, nz).to(device=device, dtype=dtype)
    ...
    return grid.reshape(nx * ny, nz, 3)  # (N_ch, Nz, 3)


PARAM_IDX = [-2, -1]  # unit_cell, theta


def predict_chi_batch_from_params(  # ruff: ignore[too-many-locals] # ruff: ignore[too-many-arguments]  # ruff: ignore[too-many-positional-arguments]
    forward_model: nn.Module,
    params_batch: torch.Tensor,
    coords: torch.Tensor,  # (Nx * Ny, Nz, 3)
    nx: int,
    ny: int,
    z: torch.Tensor = z,
) -> torch.Tensor:
    b = params_batch.shape[0]
    n_ch, nz = coords.shape[0], coords.shape[1]
    ...
    index = _make_channel_index(nx, ny, device)  # (N_ch, 2)
    local = potential_batch[:, index[:, 0], index[:, 1]]  # (B, N_ch, Nz)
    # trapezoid weights on the uniform z grid
    z_dev = z.to(device=device, dtype=dtype)
    kernel = forward_model.global_encoder(
        coords
    ...
    # the pointwise head still sees only (n, m) — constant along the z axis
    coords_2d = coords[:, 0, :2]  # (N_ch, 2)

    coords_flat = coords_2d.unsqueeze(0).expand(b, -1, -1).reshape(b * n_ch, 2)
    params_flat = params_batch[:, PARAM_IDX].repeat_interleave(n_ch, dim=0)
    global_flat = global_feature.repeat_interleave(n_ch, dim=0)
    local_flat = local.reshape(b * n_ch, nz)
...
def train() -> None:  # noqa: PLR0914   # ruff: ignore[too-many-statements]
    ...
    coords = _make_coords(Nx, Ny, potential_z, device=DEVICE, dtype=torch.float32)
    ...
    output_dir = Path("data/full_potential_model_flex/2/training_logs")
    ...
    for epoch in range(epochs):
        ...
        if average_val_loss_f < best_val_loss_f:
            ...
            torch.save(
                ...
                "data/full_potential_model_flex/2/best_forward_model_potential_full.pth",
            )
            epochs_without_improvement = 0
        else:
    ...
    torch.save(
        forward_model.state_dict(),
        "data/full_potential_model_flex/2/forward_model_potential_full.pth",
    )


OUTPUT_DIR = Path("data/full_potential_model_flex/2")
...
def test() -> None:  # ruff: ignore[too-many-locals]
    ...
    condition = MorseScatteringCondition(
        ...
        metadata=scattering_metadata_from_stacked_delta_x(
            ...
            (potential_channels, potential_channels, 100),
        ),
        incident_k=momentum_from_angles(
            theta=np.deg2rad(45),
            phi=np.deg2rad(0),
    ...
    a = 3.0 * angstrom_si
    depth = 7.63 * electron_volt * 1e-3
    height = (1.0 / 1.1) * angstrom_si
    offset = 1.0 * angstrom_si
    beta = 0.05

    metadata0 = scattering_metadata_from_stacked_delta_x(
        (
            np.array([a, 0, 0]),
            (np.array([0, a, 0])),
            np.array([0, 0, Z_HEIGHT * angstrom_si]),
    ...
    potential = as_scattering_potential(initial_potential, metadata0)
    condition0 = ScatteringCondition.from_angles(
        mass=HELIUM_MASS,
        energy=HELIUM_ENERGY,
        theta=np.deg2rad(30),
        phi=np.deg2rad(0),
    ...
    panels: list[tuple[Any, str]] = [
        ...
        (actual, "Exact (multiscat, 400 channels)"),
    ...
    fig.suptitle(
        ...
        r"$D = 7.63$ meV, $a = 3$ Å, "
        rf"Number of channels = {potential_channels**2})"
        "\n"
    ...
    plt.close(fig)
    print(f"--> Saved {OUTPUT_DIR / 'scattering_comparison_full.png'}")
...
def _trapezoid_weights(z: torch.Tensor) -> torch.Tensor:
    """Trapezoid quadrature weights for a (possibly non-uniform) z grid."""
    w = torch.zeros_like(z)
    d = z[1:] - z[:-1]
    w[:-1] += d / 2
    w[1:] += d / 2
...
def predict_chi_explicit(  # ruff: ignore[too-many-locals]
    ...
    # Potential at the TARGET grid size -- this is the line the global was
    # failing to control.
    potential_batch = torch.stack(
        [_potential_at_size(p, z, n_channels=n_side) for p in params_batch],
        dim=0,
    ).to(device=device, dtype=dtype)

    index_n = coords[:, 0, 0].long()
    index_m = coords[:, 0, 1].long()

    # Fail loudly and readably rather than as a CUDA device-side assert.
    limit_n = potential_batch.shape[1]
    limit_m = potential_batch.shape[2]
    if int(index_n.max()) >= limit_n or int(index_m.max()) >= limit_m:
        msg = (
            f"Channel index out of range: coords reach "
            f"({int(index_n.max())}, {int(index_m.max())}) but the potential is "
            f"({limit_n}, {limit_m}). n_side={n_side}."
        )
        raise IndexError(msg)

    local = potential_batch[:, index_n, index_m]  # (B, N, Nz)

    z_dev = z.to(device=device, dtype=dtype)
    w = _trapezoid_weights(z_dev)

    kernel = forward_model.global_encoder(
        coords
    )  # (N, D) # ty: ignore[call-non-callable]
    global_feature = torch.einsum("nzd,bnz,z->bd", kernel, local, w)  # (B, D)
    global_feature += forward_model.post(
        global_feature
    )  # (B, D)  # ty: ignore[call-non-callable]

    coords_2d = coords[:, 0, :2]  # (N, 2)

    coords_flat = (
        coords_2d.unsqueeze(0).expand(batch, n_pts, 2).reshape(batch * n_pts, 2)
    )
    params_flat = params_batch[:, PARAM_IDX].repeat_interleave(n_pts, dim=0)
    global_flat = global_feature.repeat_interleave(n_pts, dim=0)
    local_flat = local.reshape(batch * n_pts, -1)
    ...
    return forward_model(
        ...
        local_potential=local_flat,
        global_feature=global_flat,
    ).view(batch, n_side, n_side)


def _potential_at_size(
    params: torch.Tensor | np.ndarray,
    z: torch.Tensor,
    n_channels: int,
) -> torch.Tensor:
    """`get_potential`, with the channel count passed through."""
    if isinstance(params, torch.Tensor):
        params = params.detach().cpu().numpy()
    depth, height, offset, beta, *_ = denormalize_params(
        np.asarray(params, dtype=np.float64)
...
def _evaluate_grid(
    ...
    with torch.no_grad():
        channel_intensity_dense = predict_chi_explicit(
            forward_model,
            test_params.to(DEVICE),
            coords,
            n_side,
    ...
    metrics = compute_metrics(
        predict.raw_data.real,
        actual.raw_data.real,
        name=f"Scattering matrix ({n_side * n_side} channels)",
    )
...
def test_2() -> None:  # ruff: ignore[too-many-locals] # ruff: ignore[too-many-statements]
    ...
    forward_model.eval()
    results = {}
    global potential_channels  # ruff: ignore[global-statement]
    for n in GRID_SIZES:
        potential_channels = n
        results[n] = _evaluate_grid(forward_model, n)

    channel_counts = np.array([n * n for n in GRID_SIZES], dtype=float)
    rel_l2 = np.array([float(results[n][2]["rel_l2"]) for n in GRID_SIZES])
    np.savez(
        OUTPUT_DIR / "sweep_mesh_free.npz",
    ...
    for count, value, n_side in zip(channel_counts, rel_l2, GRID_SIZES, strict=True):
        if n_side not in ANNOTATED_GRIDS:
            continue
        ax_trend.annotate(
            rf"${n_side}\times{n_side}$"
            "\n"
            f"{value:.3f}",
            xy=(count, value),
            xytext=(0, 18),
